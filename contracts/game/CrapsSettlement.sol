...
contract CrapsSettlement is ICrapsSettlement, AccessControl, ReentrancyGuard {
    ...
    function _initializePayoutTables() private {
        // Fire bet payouts
        firePayouts[4] = 3900; // 39:1
        firePayouts[5] = 24900; // 249:1
        firePayouts[6] = 99900; // 999:1

        // Ride the Line payouts
        ridePayouts[3] = 100; // 1:1
        ridePayouts[4] = 200; // 2:1
        ridePayouts[5] = 300; // 3:1
        ridePayouts[6] = 400; // 4:1
        ridePayouts[7] = 1000; // 10:1
        ridePayouts[8] = 1500; // 15:1
        ridePayouts[9] = 2000; // 20:1
        ...
        hotRollerPayouts[6] = 30000; // 300:1

        // Different Doubles payouts
        doublesPayouts[3] = 400; // 4:1
        doublesPayouts[4] = 800; // 8:1
        doublesPayouts[5] = 3000; // 30:1
        doublesPayouts[6] = 25000; // 250:1
    }
    ...
    function settleComeOutRoll(
        ...
        if (total == 7 || total == 11) {
            ...
            totalPayout += _settleBetType(
                CrapsBetTypes.BET_DONT_PASS,
                false,
                0
            );
    ...
    function settlePointRoll(
        ...
        if (total == point) {
            ...
            // Settle Pass odds
            totalPayout += _settleOddsBet(
                CrapsBetTypes.BET_ODDS_PASS,
                true,
                point
            );
            totalPayout += _settleOddsBet(
                CrapsBetTypes.BET_ODDS_DONT_PASS,
                false,
                point
            );
        } else if (total == 7) {
        ...
        returns (uint256)
    {
        ...
        for (
            uint8 i = CrapsBetTypes.BET_YES_2;
            i <= CrapsBetTypes.BET_YES_12;
            i++
        ) {
            totalPayout += _settleBetType(i, false, 0);
        }
        for (
            uint8 i = CrapsBetTypes.BET_NO_2;
            i <= CrapsBetTypes.BET_NO_12;
            i++
        ) {
        ...
        // All hardways lose
        for (
            uint8 i = CrapsBetTypes.BET_HARD4;
            i <= CrapsBetTypes.BET_HARD10;
            i++
        ) {
            totalPayout += _settleBetType(i, false, 0);
        }
    ...
    function _settleOneRollBets(uint8 total) private returns (uint256) {
        ...
        // NEXT bets (43-53)
        for (
            uint8 i = CrapsBetTypes.BET_NEXT_2;
            i <= CrapsBetTypes.BET_NEXT_12;
            i++
        ) {
    ...
    function _settlePlaceBets(uint8 total) private returns (uint256) {
        ...
        if (yesBetType != 0) {
            uint256 multiplier = CrapsBetTypes.getPayoutMultiplier(
                yesBetType,
                0
            );
     ...
     */
    function _settleBonusBets(
        ICrapsGame.ShooterState memory shooter
    ) private returns (uint256) {
        uint256 totalPayout = 0;
        ...
        if (shooter.consecutiveWins > 0) {
            uint8 wins = shooter.consecutiveWins > 11
                ? 11
                : shooter.consecutiveWins;
            uint256 rideMultiplier = ridePayouts[wins];
            totalPayout += _settleBetType(
                CrapsBetTypes.BET_RIDE_LINE,
                true,
                rideMultiplier
            );
        }
        ...
        if (shooter.pointsMadeCount >= 2) {
            uint8 points = shooter.pointsMadeCount > 6
                ? 6
                : shooter.pointsMadeCount;
            uint256 hotMultiplier = hotRollerPayouts[points];
            totalPayout += _settleBetType(
                CrapsBetTypes.BET_HOT_ROLLER,
                true,
                hotMultiplier
            );
        }
        ...
        if (doublesCount >= 3) {
            ...
            totalPayout += _settleBetType(
                CrapsBetTypes.BET_DIFFERENT_DOUBLES,
                true,
                doublesMultiplier
            );
        }
    ...
    /**
     * @notice Settle all bets of a specific type for all active bot players
     * @param betType The type of bet to settle
     * @param won Whether the bet won
     * @param multiplier Payout multiplier (100 = 1:1, 200 = 2:1, etc.)
     */
     ...
     * @param betType The type of bet to push
     */
     ...
     */
    function getFieldPayout(
        uint8 total
    ) external pure override returns (uint256) {
        if (total == 2) return 200; // 2:1
        if (total == 12) return 300; // 3:1
        if (
            total == 3 || total == 4 || total == 9 || total == 10 || total == 11
        ) {
            return 100; // 1:1
        }
        return 0; // Loses
    ...
    function isHardway(
        ...
        return
            die1 == die2 &&
            (total == 4 || total == 6 || total == 8 || total == 10);
    }
}
