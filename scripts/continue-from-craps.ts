// Continue Base Sepolia Deployment from CrapsGame
...
async function main() {
  console.log("🚀 Continuing deployment from CrapsGame...\n");
  ...
  try {
    // Deploy CrapsGame - note the correct parameter order
    console.log("📦 Deploying CrapsGame...");
    const crapsGame = await viem.deployContract("CrapsGame", [
      VRF_COORDINATOR_ADDRESS,
      BigInt(VRF_SUBSCRIPTION_ID), // subscription ID as uint64
      VRF_KEY_HASH // keyHash as bytes32
    ]);
    deployedContracts.CrapsGame = crapsGame.address;
    console.log(`✅ CrapsGame: ${crapsGame.address}`);
    console.log(`   ⚠️ Add to VRF subscription ID: ${VRF_SUBSCRIPTION_ID}\n`);
    ...
    console.log("📦 Deploying CrapsSettlement (warning: large contract)...");
