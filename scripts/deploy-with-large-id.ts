// Continue Base Sepolia Deployment with Large Subscription ID
...
// This appears to be a uint256 subscription ID, not uint64
...
async function main() {
  console.log("🚀 Continuing deployment with modified VRF handling...\n");
  ...
  try {
    // For testing, let's use a dummy subscription ID that fits in uint64
    // We'll need to update this after deployment
    const TEMP_SUBSCRIPTION_ID = 1n; // Temporary ID for deployment
    
    console.log("⚠️  Using temporary subscription ID for deployment: 1");
    console.log("   You'll need to update this after deployment\n");

    // Deploy CrapsGame with temporary subscription ID
    console.log("📦 Deploying CrapsGame (with temp subscription ID)...");
    const crapsGame = await viem.deployContract("CrapsGame", [
      VRF_COORDINATOR_ADDRESS,
      TEMP_SUBSCRIPTION_ID, // Using temporary ID
      VRF_KEY_HASH
    ]);
    deployedContracts.CrapsGame = crapsGame.address;
    console.log(`✅ CrapsGame: ${crapsGame.address}`);
    console.log(`   ⚠️ IMPORTANT: Update subscription ID after deployment!\n`);
    ...
    console.log("\n📦 Deploying GachaMintPass (with temp subscription ID)...");
    ...
    console.log("✅ Treasury role granted to Treasury");
    ...
    const deploymentInfo = {
      ...
      vrfSubscriptionIdUsed: "1 (temporary - needs update)",
      contracts: deployedContracts,
      notes: [
        "IMPORTANT: VRF subscription ID needs to be updated after deployment",
        `Your actual subscription ID: ${VRF_SUBSCRIPTION_ID}`,
        "The contracts were deployed with subscription ID: 1 (temporary)",
        "You need to call setSubscriptionId() on CrapsGame and GachaMintPass"
      ]
    };
    ...
    console.log("════════════════════════════════════");
    console.log("1. UPDATE VRF SUBSCRIPTION ID:");
    console.log("   The contracts were deployed with temporary subscription ID: 1");
    console.log(`   Your actual subscription ID: ${VRF_SUBSCRIPTION_ID}`);
    console.log("   You need to update this via contract functions or redeploy");
    
    console.log("\n2. ADD CONTRACTS AS VRF CONSUMERS:");
    console.log(`   - CrapsGame: ${crapsGame.address}`);
    console.log(`   - GachaMintPass: ${gachaMintPass.address}`);
    console.log(`   Go to: https://vrf.chain.link/base-sepolia`);

    const finalBalance = await publicClient.getBalance({ address: account.address });
    console.log(`\n💰 Deployment Cost: ${formatEther(balance - finalBalance)} ETH`);
