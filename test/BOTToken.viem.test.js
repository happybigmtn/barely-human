...
describe("BOTToken", function () {
  ...
  describe("Transfers", function () {
    ...
    it("Should allow approve and transferFrom", async function () {
      const { botToken, treasury, user1, user2 } = await loadFixture(deployBOTTokenFixture);
      ...
      await botToken.write.transferFrom(
  ...
  describe("Pausable", function () {
    it("Should allow pauser to pause and unpause", async function () {
      const { botToken, deployer } = await loadFixture(deployBOTTokenFixture);
      
      // Initially not paused
      expect(await botToken.read.paused()).to.be.false;
      
      // Pause
      await botToken.write.pause({ account: deployer.account });
      expect(await botToken.read.paused()).to.be.true;
      
      // Unpause
      await botToken.write.unpause({ account: deployer.account });
      expect(await botToken.read.paused()).to.be.false;
    });

    it("Should prevent transfers when paused", async function () {
      const { botToken, deployer, treasury, user1 } = await loadFixture(deployBOTTokenFixture);
  ...
  describe("Burn", function () {
    it("Should allow token holders to burn their tokens", async function () {
      const { botToken, treasury } = await loadFixture(deployBOTTokenFixture);
  ...
  describe("Access Control", function () {
    it("Should allow admin to grant roles", async function () {
      const { botToken, deployer, user1 } = await loadFixture(deployBOTTokenFixture);
    ...
    it("Should allow admin to revoke roles", async function () {
      const { botToken, deployer, treasury } = await loadFixture(deployBOTTokenFixture);
      
      // Revoke TREASURY_ROLE from treasury
      await botToken.write.revokeRole(
